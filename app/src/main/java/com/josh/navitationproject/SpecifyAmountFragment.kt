...
//        val money = SpecifyAmountFragment.fromBundle(arguments).money
        arguments?.let {
            val safeArgs = SpecifyAmountFragmentArgs.fromBundle(it)
            val money = safeArgs.money
            recipient = money!!.name
        }
        ...
        when(v!!.id){
            R.id.send_btn -> {
                if(!TextUtils.isEmpty(input_amount.text.toString())){
                    ...
                    val amount = Money(input_amount.text.toString(), "")
                    val bundle = bundleOf(
                        "recipient" to recipient,
                        "amount" to amount
                    )
                    navController!!.navigate(
                        R.id.action_specifiedAmountFragment_to_confirmationFragment,
                        bundle
                    )
                }
                else{
